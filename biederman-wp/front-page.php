<?php
/**
 * Front Page template (block-based).
 * This template supports both block editor content and traditional PHP templates.
 */
if (!defined('ABSPATH')) { exit; }
get_header();

// Check if the page has block content
$page = get_post(get_option('page_on_front'));
$has_blocks = $page && has_blocks($page->post_content);
?>
...
<main id="main">
  ...
  <?php else: ?>
    <?php
      // Fallback: Use traditional template structure
      // This can be replaced with block patterns or removed entirely
      // if you want to use only blocks
      ...
      <div class="container">
        ...
        <div class="cards">
          <?php
            ...
            $featured_show = biederman_get_featured_show();
            if ($featured_show):
              global $post;
              $post = $featured_show;
              setup_postdata($post);
          ...
          <?php
            ...
            $upcoming_shows = biederman_get_upcoming_shows(3);
            if ($upcoming_shows->have_posts()):
              while ($upcoming_shows->have_posts()):
                $upcoming_shows->the_post();
                get_template_part('template-parts/content', 'show');
              endwhile;
              wp_reset_postdata();
            else:
    ...
    <section id="press" class="section section--alt">
      <div class="container">
        <div class="section__head">
          <h2><?php esc_html_e('Presse & Press Assets', 'biederman'); ?></h2>
        ...
        <div class="cards">
          ...
          <?php
            // Display Press Assets
            $press_assets = biederman_get_press_assets();
            if ($press_assets->have_posts()):
              while ($press_assets->have_posts()):
                $press_assets->the_post();
